...
from .notifications import AlertManager, WebhookNotifier
from .reporting import ReportGenerator, EmailReporter
...
class Flask(App):
    ...
    def __init__(
        ...
        # 모니터링 기능 초기화
        self._monitoring = MonitoringCollector()
        self._monitoring_enabled = True

        # 알림 기능 초기화
        self._alert_manager = AlertManager()
        self._webhook_notifier = None  # application.py에서 설정
    ...
    def setup_weekly_reporting(
        ...
        try:
            ...
            print("⚠️ APScheduler가 설치되지 않았습니다. 'pip install apscheduler'로 설치하세요.")
            return
        
        self._report_generator = ReportGenerator(self._monitoring)
        self._email_reporter = EmailReporter(
            smtp_server=smtp_server,
            smtp_port=smtp_port,
            sender_email=sender_email,
            sender_password=sender_password
        )
        self._report_recipients = recipients
        ...
        day_of_week = day_map.get(schedule_day.lower(), "mon")
        hour, minute = map(int, schedule_time.split(":"))
        
        scheduler.add_job(
            func=self._generate_and_send_weekly_report,
        ...
        print(f"✅ 주간 이메일 보고서가 활성화되었습니다.")
        print(f"   스케줄: 매주 {schedule_day} {schedule_time}")
        print(f"   수신자: {', '.join(recipients)}")
    
    def _generate_and_send_weekly_report(self) -> None:
        """주간 리포트 생성 및 발송 (스케줄러에서 호출)"""
        if not self._reporting_enabled or not self._report_generator or not self._email_reporter:
            return
        ...
        try:
            ...
            report = self._report_generator.generate_weekly_report(last_monday, last_sunday)
            html_content = self._report_generator.format_report_html(report)
            text_content = self._report_generator.format_report_text(report)
            
            success = self._email_reporter.send_weekly_report(
            ...
            if success:
                print(f"✅ 주간 리포트가 성공적으로 발송되었습니다.")
            else:
                print(f"⚠️ 주간 리포트 발송에 실패했습니다.")
        except Exception as e:
            print(f"❌ 주간 리포트 생성/발송 중 오류 발생: {e}")
            import traceback
            traceback.print_exc()
    
    def send_test_report(self) -> bool:
        """테스트 리포트를 즉시 발송합니다 (비동기 처리)."""
        if not self._reporting_enabled or not self._report_generator or not self._email_reporter:
            print("⚠️ 주간 리포트 기능이 설정되지 않았습니다.")
            return False
        ...
        def send_report_async():
            try:
                print(f"\n📧 테스트 리포트 발송을 시작합니다...")
                print(f"   수신자: {', '.join(self._report_recipients)}")
                print(f"   발신자: {self._email_reporter.sender_email}")
                print(f"   SMTP 서버: {self._email_reporter.smtp_server}:{self._email_reporter.smtp_port}")
                
                end_date = datetime.now()
                start_date = end_date - timedelta(days=7)
                
                print(f"   리포트 기간: {start_date.strftime('%Y-%m-%d')} ~ {end_date.strftime('%Y-%m-%d')}")
                
                print(f"   리포트 생성 중...")
                try:
                    report = self._report_generator.generate_weekly_report(start_date, end_date)
                    print(f"   리포트 생성 완료: 총 {report.total_requests}건의 요청")
                except Exception as e:
                    print(f"   ❌ 리포트 생성 오류: {e}")
                    import traceback
                    traceback.print_exc()
                ...
                if success:
                    print(f"✅ 테스트 리포트가 성공적으로 발송되었습니다.")
                    print(f"   이메일을 확인하세요: {', '.join(self._report_recipients)}")
                    print(f"   스팸 폴더도 확인해보세요.\n")
                else:
                    print(f"⚠️ 테스트 리포트 발송에 실패했습니다.")
                    print(f"   위의 오류 메시지를 확인하세요.\n")
            except Exception as e:
                print(f"❌ 테스트 리포트 생성/발송 중 오류 발생: {e}")
                import traceback
                traceback.print_exc()
        
        # 백그라운드 스레드에서 실행 (블로킹 방지)
        thread = threading.Thread(target=send_report_async, daemon=True)
        thread.start()
        
        # 즉시 성공 반환 (실제 발송은 백그라운드에서 진행)
        return True

    def get_monitoring_stats(self) -> dict[str, t.Any]:
        """모니터링 통계 데이터를 반환합니다."""
        if not self._monitoring_enabled:
            return {"error": "Monitoring is disabled"}
        return self._monitoring.get_statistics()

    def get_request_metrics(self, limit: int = 100) -> list[dict[str, t.Any]]:
        """최근 요청 메트릭을 반환합니다."""
        if not self._monitoring_enabled:
            return []
        return self._monitoring.get_recent_requests(limit=limit)

    def get_error_rate(self, time_window: int = 60) -> float:
        """지정된 시간 윈도우 내의 오류 발생 비율을 반환합니다."""
        if not self._monitoring_enabled:
            return 0.0
        return self._monitoring.get_error_rate(time_window=time_window)

    def get_average_response_time(self, time_window: int = 60) -> float:
        """지정된 시간 윈도우 내의 평균 응답 시간을 반환합니다."""
        if not self._monitoring_enabled:
            return 0.0
        return self._monitoring.get_average_response_time(time_window=time_window)

    def get_endpoint_stats(self) -> dict[str, dict[str, t.Any]]:
        """엔드포인트별 통계를 반환합니다."""
        if not self._monitoring_enabled:
            return {}
        return self._monitoring.get_endpoint_stats()

    def enable_monitoring(self, enabled: bool = True) -> None:
        """모니터링 기능을 활성화/비활성화합니다."""
        self._monitoring_enabled = enabled

    def setup_webhook_notifier(self, webhook_url: Optional[str] = None, enabled: bool = True) -> None:
        """웹훅 알림을 설정합니다."""
        self._webhook_notifier = WebhookNotifier(webhook_url=webhook_url, enabled=enabled)

    def enable_notifications(self, enabled: bool = True) -> None:
        """알림 기능을 활성화/비활성화합니다."""
        self._notifications_enabled = enabled

    def get_recent_alerts(self, limit: int = 10, unread_only: bool = False) -> list[dict[str, t.Any]]:
        """최근 알림 목록을 반환합니다."""
        if not self._notifications_enabled:
            return []
        return self._alert_manager.get_recent_alerts(limit=limit, unread_only=unread_only)

    def mark_alert_read(self, alert_id: str) -> bool:
        """알림을 읽음 처리합니다."""
        if not self._notifications_enabled:
            return False
        return self._alert_manager.mark_alert_read(alert_id)

    def get_unread_alert_count(self) -> int:
        """읽지 않은 알림 수를 반환합니다."""
        if not self._notifications_enabled:
            return 0
        return self._alert_manager.get_unread_count()
